...
const FragmentCard = ({ fragment, isActiveFragment, onFragmentClick }: FragmentCardProps) => {
  return (
    <button
      className={cn(
        'mg-muted hover:bg-secondary flex w-fit items-start gap-2 rounded-lg border p-3 text-start transition-colors',
...
const AssistantMessage = ({
  ...
  return (
    <div
      className={cn(
        'group flex flex-col px-2 pb-2',
        type === 'ERROR' && 'text-red-600 dark:text-red-500'
      )}
    >
      <div className="mb-2 flex items-center gap-2 pl-2">
        <Image src="/logo.svg" width={18} height={18} className="shrink-0" alt="Nectar" />
